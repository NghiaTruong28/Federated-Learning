...
def newMedian(epoch, gradients, net, lr, perturbation, f=0, byz=no_byz):
    ...
    m = n - b*2
    median_nd = nd.mean(sorted_array[:, b:(b+m)], axis=-1, keepdims=1)
    mean_around_median_nd = nd.sum(sorted_array * nd.topk(nd.abs(sorted_array-median_nd), ret_typ='mask', k=(n-b), is_ascend=1, axis=-1), axis=-1) / float(n-b)
    return mean_around_median_nd
